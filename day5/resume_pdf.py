...
Do NOT return field like "propertiese","type" or "title".
Fill the schema with actual information extracted from the job description.

If minium experience is not mentioned, return NULL.
...
user_prompt = f"""
Analyze the following jon description:
{job_description}
"""
...
def final_score(job,resume):
    ...
    Give me:
    1.Cadidate name
    2.Matching skills must contain only skils that are present in candidate resume.
    3.Missing important skillsmust contain skills that required but not present in candidate resume.
    4.whether experience requirement is met or not.
    5.Overall match percantage from 0 to 100
    6.A short final verdict
    7.do not give 100 % unless the candidateactually matches the requiremwnt.
    8.Do not put job requirements into matching skills

    Keep the response consice and easy to read.
...
def parse_resume(resume_text):
    print("RESUME TEXT RECEIVED:",resume_text[1000])
    resume_schema = Resume.model_json_schema()
    print("resume schema:",resume_schema)

    system_prompt = f"""
    You are an expert resume parser.

    Exteract information fom the resume based on its meaning
    ...
    Skills may also appear in the skill section, work experience,
    ...
    2.If a value is not availabel return null.
    3.If a list has no value , retrn an empty list.
    4.Include internships inside the experience.
    5.Extract skills mention inside the resume.
    6.calcualte total_experience _years from the experience date . If an experience is ongoing (for example,"2023 - present"),calaculate the experience uoto the current date.
